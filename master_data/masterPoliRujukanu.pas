...
type
  TForm1_masterPoliRujukan = class(TForm)
    panelUtama: TJvPanel;
    JvPanel1: TJvPanel;
...
var
  Form1_masterPoliRujukan: TForm1_masterPoliRujukan;

implementation

uses dmMaster, aau, aau111, brPoliU; //, srvPoliU;

{$R *.dfm}

{ TForm1_satuan }

procedure TForm1_masterPoliRujukan.awal;
begin
...
procedure TForm1_masterPoliRujukan.btn_closeClick(Sender: TObject);
begin
...
procedure TForm1_masterPoliRujukan.btn_mutasiClick(Sender: TObject);
var jkn : brPoli;
    //srv : srvPoli;
begin
// ambil dari jkn dulu
jkn := brPoli.Create;
if jkn.getPoliFktl('0', '200') then
begin
  ShowMessage('jkn ok');
...
procedure TForm1_masterPoliRujukan.buka_data;
begin
with dataMaster do
begin
  FDConnection1.Connected := false;
  fdQueryPoliRujukan.Open;
end;
end;

procedure TForm1_masterPoliRujukan.FormShow(Sender: TObject);
begin
awal;
end;

procedure TForm1_masterPoliRujukan.nav1Click(Sender: TObject; Button: TNavigateBtn);
  var apv : string;
begin
...
begin
  apv := dataMaster.fdQueryPoliRujukan.FieldByName('kdpoli').AsString;
  grid1.Options := grid1.Options - [dgEditing] + [dgRowSelect];
