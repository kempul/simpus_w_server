...
type
  TForm1_Prolanis = class(TForm)
    ...
    procedure JvDBNavigator1BeforeAction(Sender: TObject; Button: TNavigateBtn);
  private
...
procedure TForm1_Prolanis.dbnav2BeforeAction(Sender: TObject;
  ...
  var
    ...
    status : string;
begin
status := TJvDBNavigator(Sender).DataSource.DataSet.FieldByName('status').AsString;
if Button = nbDelete then
if MessageBox(0, 'Hapus Data?', 'Konfirmasi', MB_YESNO) = mrYes then
begin
   if LowerCase(status) = 'ok' then
   begin
   hasil := False;
   br0 := brProlanis.Create;
   try
...
procedure TForm1_Prolanis.JvDBNavigator1BeforeAction(Sender: TObject;
  Button: TNavigateBtn);
  var br0 : brProlanis;
      length_edu_id : integer;
begin
   length_edu_id := Length(TJvDBNavigator(Sender).DataSource.DataSet.FieldByName('edu_id').AsString);

if Button = nbDelete then
begin
   if MessageBox(0, 'Yakin Hapus?', 'Konfirmasi', MB_YESNO) = mrYes then
   begin
     if length_edu_id > 0 then begin
     br0 := brProlanis.Create;
     try
       br0.delKegiatan(dataProlanis.fdQKegiatan.FieldByName('id').AsString);
       if not br0.adl_berhasil then Abort;
