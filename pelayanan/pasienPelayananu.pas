...
type
  ...
  private
    ...
    slPoliAntrian: TStringList;
    //kd_tkp : string;

    //function cek_mcu(idxstr : )
...
uses dateUtils, aau, aau111, dmpelayanan, pulangBantuU, ICDXCariu, gigiBantuU,
  ...
  brObatU, SrvBpjsParallelU, OtlParallel,
   faskesRujukanU, lab_bpjs; //SrvParallelU, srvIcdxU, srvDiagnosaU, srvFisikU, srvPulangU,
...
begin
Form1_labBpjs.lblPasien.Caption := dbtxt1.Field.AsString;
Form1_labBpjs.lbl1Idxstr.Caption := dbeditIdxstr.Field.AsString;
Form1_labBpjs.dtPicker1.Date := dtp1.date;
Form1_labBpjs.chk1Bpjs.Checked := checkBPJS.Checked;
form1_labBpjs.ShowModal;
...
var vPoli, vWav : string;
    //indek : Integer;
begin
vpoli := LowerCase(slPoliAntrian.Names[cbb1PoliAntri.ItemIndex]);
vWav := LowerCase(slPoliAntrian.ValueFromIndex[cbb1PoliAntri.ItemIndex]);

//LowerCase(cbb1PoliAntri.Items[cbb1PoliAntri.ItemIndex]);
putarSuara(spinAntri.Value,  vWav);
...
begin
boleh_buka := False;
slPoliAntrian := TStringList.Create;
end;

procedure TForm1_pelayanan.FormDestroy(Sender: TObject);
begin
FreeAndNil(slPoliAntrian);
end;
...
procedure TForm1_pelayanan.isiPoliAntrian;
var i: Integer;
begin
slPoliAntrian.Clear;
with dataPelayanan do
begin
  fdQMasterPoliAntrian.Close;
  fdQMasterPoliAntrian.Open();

  while not fdQMasterPoliAntrian.Eof do
  begin
    slPoliAntrian.Add(fdQMasterPoliAntrian.FieldByName('poli_antrian').AsString + '=' +
      fdQMasterPoliAntrian.FieldByName('wav').AsString);
    fdQMasterPoliAntrian.Next;
  end;
  ...
  // isi pada comboBox
  cbb1PoliAntri.Clear;
for I := 0 to slPoliAntrian.Count - 1 do
  begin
     cbb1PoliAntri.Items.Add(slPoliAntrian.Names[i]);
  end;
...
begin
  ...
  //ShowMessage(VariantSaveJSON(v0));
end;
